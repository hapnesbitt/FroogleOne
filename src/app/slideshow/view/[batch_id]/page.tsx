...
// const IMAGE_DISPLAY_DURATION = 15000; // Removed - was unused

export default function PublicSlideshowPage() {
  const params = useParams();
  const shareToken = params.batch_id as string; // Changed from share_token to batch_id as per route
  ...
  useEffect(() => {
    ...
    // let imageTimer: NodeJS.Timeout | null = null; // Removed - was unused and caused `let` to `const` warnings
  ...
  if (!batch || mediaData.length === 0) {
    return (
      <div className="p-8 text-center text-xl font-semibold text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 rounded-lg shadow-md">
        No slideshow available or no playable media found in this Lightbox for slideshow.
      </div>
    );
  }
