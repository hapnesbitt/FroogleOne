...
# --- NEW IMPORTS FOR JWT (FINAL CORRECTION) ---
from flask_jwt_extended import JWTManager, create_access_token, jwt_required, get_jwt_identity, verify_jwt_in_request
# FINAL CORRECTED IMPORT: Removed 'MissingRequiredClaimError'
...
# --- COMMENT OUT/REMOVE THESE LINES - NOT USED FOR JWT AUTH ---
# app.config['SESSION_COOKIE_HTTPONLY'] = True
# app.config['SESSION_COOKIE_SAMESITE'] = 'None'
# app.config['SESSION_COOKIE_SECURE'] = os.environ.get('SESSION_COOKIE_SECURE', 'False').lower() == 'true'

# --- JWT Configuration ---
app.config['JWT_SECRET_KEY'] = os.environ.get('JWT_SECRET_KEY', 'super-secret-jwt-key-CHANGE_ME_IN_PROD')
app.config['JWT_ACCESS_TOKEN_EXPIRES'] = datetime.timedelta(hours=int(os.environ.get('JWT_EXPIRATION_HOURS', 24)))
...
def invalid_token_callback(callback_error):
    app.logger.warning(f"API: Invalid token: {callback_error}")
    return jsonify(success=False, message="Invalid token. Please log in again."), 401

@jwt.unauthorized_loader
def unauthorized_callback(callback_error):
    app.logger.warning(f"API: Unauthorized token: {callback_error}")
    return jsonify(success=False, message="Authorization header missing or malformed. Please log in."), 401

@jwt.expired_token_loader
def expired_token_callback(jwt_header, jwt_payload):
    app.logger.warning(f"API: Expired token for user: {jwt_payload.get('sub')}")
...
app.config['MAX_CONTENT_LENGTH'] = int(os.environ.get('MAX_CONTENT_LENGTH', 9000 * 1024 * 1024))
...
CORS(
    ...
    resources={r"/api/v1/*": {"origins": [
        "http://localhost:3000",
        "http://127.0.0.0:3000",
        "https://lightbox.mine.nu",
        "https://vibe.mine.nu",
        "https://vibeapi.mine.nu"
    ]}},
...
app.logger.info("CORS configured for /api/v1/* with specified origins, headers, and methods.")
...
def get_unique_disk_path(directory, base, ext_dot):
...
# --- API Decorators (MODIFIED FOR JWT) ---
def login_required_api(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            request.current_identity = get_jwt_identity()
        # FINAL CORRECTED: Catch specific exceptions from Flask-JWT-Extended 4.x
        except (NoAuthorizationError, InvalidHeaderError) as e:
            app.logger.warning(f"API: JWT Unauthorized access attempt to {request.path}: {e}")
            return jsonify(success=False, message="Authentication required. Please log in."), 401
        except Exception as e: # Catch any other unexpected errors during JWT verification
            app.logger.error(f"API: Unexpected JWT verification error: {e}", exc_info=True)
...
def admin_required_api(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            if not redis_client:
                app.logger.error(f"API: Admin access check: Redis client not available for user '{current_username}'.")
                return jsonify(success=False, message="Service temporarily unavailable."), 503

            user_redis_data = redis_client.hgetall(f'user:{current_username}')
            if not user_redis_data or user_redis_data.get('is_admin') != '1':
                app.logger.warning(f"API: User '{current_username}' attempted admin access without privileges on {request.path}.")
                return jsonify(success=False, message="Admin privileges required."), 403
            
            request.current_identity = current_username
            return f(*args, **kwargs)
        # FINAL CORRECTED: Catch specific exceptions from Flask-JWT-Extended 4.x
        except (NoAuthorizationError, InvalidHeaderError) as e:
            app.logger.warning(f"API: Admin access without valid JWT: {e}")
            return jsonify(success=False, message="Authentication required."), 401
        except Exception as e:
            app.logger.error(f"API: Unexpected error during admin check: {e}", exc_info=True)
            return jsonify(success=False, message="Authentication error."), 500
    return decorated_function
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # FINAL CORRECTED: Catch specific exceptions from Flask-JWT-Extended 4.x
                ...
                app.logger.warning(f"API: Owner/Admin check: Unauthorized access attempt to {request.path} for {item_type}: {e}")
                return jsonify(success=False, message="Authentication required."), 401
            except Exception as e:
                app.logger.error(f"API: Unexpected error during owner/admin check: {e}", exc_info=True)
                return jsonify(success=False, message="Authentication error."), 500
            ...
            is_admin = redis_client.hget(f'user:{current_username}', 'is_admin') == '1'
            if item_owner != current_username and not is_admin:
                app.logger.warning(f"API: User '{current_username}' attempted unauthorized access to {item_type} '{item_id_to_check_str}' owned by '{item_owner}'.")
...
def api_auth_status_check():
    ...
    try:
        ...
        if current_username:
            if not redis_client:
                app.logger.error(f"API: Redis not available for auth status check of user '{current_username}'.")
                return jsonify(isLoggedIn=False, user=None, message="Service unavailable to verify authentication."), 503

            user_redis_data = redis_client.hgetall(f'user:{current_username}')
            if user_redis_data:
                user_info = {"username": current_username, "isAdmin": user_redis_data.get('is_admin') == '1'}
                app.logger.info(f"API: GET {API_PREFIX}/auth/status - User '{current_username}' IS logged in via JWT. Admin: {user_info['isAdmin']}")
                return jsonify(isLoggedIn=True, user=user_info, message="User is authenticated."), 200
            else:
                app.logger.warning(f"API: GET {API_PREFIX}/auth/status - Valid JWT for non-existent Redis user: '{current_username}'.")
                return jsonify(isLoggedIn=False, user=None, message="Invalid user or authentication expired."), 401
        else:
            app.logger.info(f"API: GET {API_PREFIX}/auth/status - No valid JWT found.")
            return jsonify(isLoggedIn=False, user=None, message="User is not authenticated."), 200
    except Exception as e:
        app.logger.error(f"API: Unexpected error during auth status check: {e}", exc_info=True)
        return jsonify(isLoggedIn=False, user=None, message="An error occurred during authentication check."), 500
    ...
    try:
        ...
        if check_password_hash(stored_password_hash, password):
            ...
            user_info_for_frontend = {
                "username": username,
                "isAdmin": user_redis_data.get('is_admin') == '1'
            }
            app.logger.info(f"API: POST {API_PREFIX}/auth/login - Login SUCCESSFUL for user: '{username}'. Admin: {user_info_for_frontend['isAdmin']}")
...
def api_logout():
    ...
    app.logger.info(f"API: POST {API_PREFIX}/auth/logout - Frontend initiated logout.")
...
def api_register():
    ...
    try:
        ...
        current_username = get_jwt_identity()
        if current_username:
            app.logger.warning(f"API: User '{current_username}' attempted to register while already logged in.")
            return jsonify(success=False, message="Already logged in. Please log out first."), 409
    except Exception as e:
        app.logger.debug(f"API: No valid JWT during register check (expected if not logged in): {e}")
...
def api_list_batches():
    ...
    username = request.current_identity
...
def api_create_batch():
    ...
    current_username = request.current_identity
    ...
    for mid in media_ids:
        ...
        if mdata_raw:
            media_item = {
                ...
                'filepath': mdata_raw.get('filepath'),
            ...
            if media_item['filepath'] and media_item['processing_status'] == 'completed':
                ...
                media_item['web_url'] = None
            media_list.append(media_item)
        else:
            app.logger.warning(f"API: Media ID {mid} in batch {batch_id_str} but no data in Redis.")

    batch_info['media_items'] = media_list
    batch_info['item_count'] = len(media_ids)
    batch_info['playable_media_count'] = sum(1 for item in media_list if item['web_url'] and item['item_type'] == 'media' and not item['is_hidden'])

    app.logger.info(f"API: User '{request.current_identity}' fetched details for batch '{batch_id_str}'.")
...
def api_toggle_share_batch(batch_id, batch_data):
    ...
    if new_shared:
        ...
        if token:
            pipe.delete(f'share_token:{token}')
            update['share_token'] = ''
    ...
    try:
        ...
        app.logger.info(f"API: User '{request.current_identity}' changed batch '{name}' (ID: {batch_id_str}) sharing to: {'Public' if new_shared else 'Private'}")
...
def api_rename_batch(batch_id, batch_data):
    ...
    try:
        ...
        current_app.logger.info(f"API: Batch '{old_name}' (ID: {batch_id_str}) renamed to '{new_name}' by {request.current_identity}")
...
@app.route(f'{API_PREFIX}/batches/<uuid:batch_id>', methods=['DELETE', 'OPTIONS'])
@login_required_api
@owner_or_admin_access_required_api(item_type='batch')
def api_delete_batch(batch_id, batch_data):
    ...
    try:
        ...
        return jsonify(success=True, message=f'Lightbox "{name_flash}" and its contents deleted.'), 200
...
def api_upload():
    ...
    current_user = request.current_identity
    ...
    if existing_batch_id:
        ...
        try:
            ...
            is_admin = redis_client.hget(f'user:{current_user}', 'is_admin') == '1'
            if batch_owner != current_user and not is_admin:
    ...
    full_disk_dir = os.path.join(app.config['UPLOAD_FOLDER'], disk_path_segment)
        ...
        try:
            if upload_type == 'import_zip' and ext_no_dot == 'zip':
                ...
                if ext_no_dot in vid_formats:
                    ...
                    final_path, final_name = get_unique_disk_path(full_disk_dir, sec_base, ext_dot)
                    file_item.save(final_path)
                    redis_pipe.hset(f'media:{item_id}', mapping={
                        **common_data, 
                        'filename_on_disk': final_name,
                        'filepath': os.path.join(disk_path_segment, final_name),
                        'processing_status': 'completed'
...
@app.route(f'{API_PREFIX}/media/<uuid:media_id>', methods=['DELETE', 'OPTIONS'])
@login_required_api
@owner_or_admin_access_required_api(item_type='media')
def api_delete_media(media_id, media_data):
    ...
    media_id_str = str(media_id)
    batch_id_contained_in = media_data.get('batch_id')
    ...
    try:
        ...
        return jsonify(
            ...
            media_id=media_id_str,
            batch_id=batch_id_contained_in
        ), 200
...
def api_display_media_item(media_id, media_data):
    ...
    app.logger.info(f"API: User '{request.current_identity}' serving/displaying '{orig_fname}' (ID: {media_id})")
    try:
...
def api_download_media_item(media_id, media_data):
    ...
    app.logger.info(f"API: User '{request.current_identity}' downloading '{orig_fname}' (ID: {media_id})")
    try:
...
def api_export_batch(batch_id, batch_data):
    ...
    try:
        ...
        app.logger.info(f"API: User '{request.current_identity}' exporting {files_in_zip} items from batch '{batch_id_str}'.")
...
def api_public_slideshow_view(share_token):
    ...
    try:
        ...
        if not js_media_list:
            return jsonify(success=False, message="No playable media items available for slideshow in this Lightbox."), 404
...
def api_admin_dashboard():
    ...
    try:
        ...
        for uname in sorted(list(usernames),key=lambda s:s.lower()):
            ...
            uinfo['is_admin'] = uinfo.get('is_admin', '0') == '1'
...
def api_change_user_password():
    ...
    try:
        ...
        app.logger.info(f"API: Admin '{request.current_identity}' changed password for '{target_user}'.")
...
# --- Consolidated JSON Error Handlers ---
